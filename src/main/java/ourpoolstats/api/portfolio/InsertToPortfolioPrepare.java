...
public class InsertToPortfolioPrepare {
	...
	public ResponseEntity<ResponsePortfolio> insertToPortfolio(Portfolio portfolio) {
		...
		try {
			SetConnection.getInstance().getJdbcTemplate().update(QueryPortfolio.getInstance().getInsertToPotfolio(),portfolio.getUsername(),portfolio.getUserType(),portfolio.getNameCoin(),portfolio.getPriceInitial(),portfolio.getPriceCurrent(),portfolio.getQuantity());
